...
# 強制把本檔所在目錄加到 module search path
...
def read_params(args):
    ...
    parser.add_argument('--colors', dest='colors', type=str, default='',
                        help="Comma-separated list of colors for classes in sorted order, e.g. 'red,blue,green'")
...
def read_data(input_file,params):
    ...
    tree['classes'].sort()
    # 依照 --colors 參數產生 class_to_color_map
    def get_color_map(classes, colors_arg):
        if colors_arg:
            cols = [c.strip() for c in colors_arg.split(',')]
            if len(cols) != len(classes):
                raise ValueError(f"--colors 需要 {len(classes)} 個顏色，你給了 {len(cols)}")
            return dict(zip(classes, cols))
        cycle = plt.rcParams['axes.prop_cycle'].by_key()['color']
        return {c: cycle[i % len(cycle)] for i,c in enumerate(classes)}

    tree['class_to_color_map'] = get_color_map(tree['classes'], params.get('colors',''))
    all_nodes = [CladeNode("root."+row[0],float(row[1])) for row in rows]

    # --- 修改開始 ---
    # 根據 params['class_colors'] 建立類別到顏色的映射
    class_to_color_map = {}
    if params['colors']:
        user_colors_list = [c.strip() for c in params['colors'].split(',')]
        if len(user_colors_list) != len(tree['classes']):
            raise ValueError(f"For cladogram --colors requires {len(tree['classes'])} values (for classes {tree['classes']}), but you provided {len(user_colors_list)}.")
        class_to_color_map = dict(zip(tree['classes'], user_colors_list))
    else:
        # 如果沒有提供自定義顏色，則使用預設的循環顏色
        for i, cls in enumerate(tree['classes']):
            class_to_color_map[cls] = colors[i % len(colors)]
    tree['class_to_color_map'] = class_to_color_map # 將映射儲存在 tree 中，供 draw_tree 使用
    # --- 修改結束 ---
    ...
    for i,v in enumerate(rows):
        if len(v)>2:
            ...
            elif v[2].count('rgbcol') > 0:
                ccc = [float(tt) for tt in v[2].split('_')[1:]]
                all_nodes[i].set_color(ccc)
            else:
                # --- 修改開始 ---
                # 使用 class_to_color_map 來設定節點顏色
                all_nodes[i].set_color(class_to_color_map[v[2]])
                # --- 修改結束 ---
...
def plot_names(father,params,depth,ax,u_i,seps):
    ...
    l = len(father.name)
    if len(children)==0:
        if isinstance(father.prev_leaf, CladeNode) and isinstance(father.next_leaf, CladeNode):
            fr_0, fr_1 = (father.pos[0] + father.prev_leaf.pos[0]) * 0.5, \
                 (father.pos[0] + father.next_leaf.pos[0]) * 0.5
        else:
            fr_0, fr_1 = father.pos[0], father.pos[0]  # fallback 或其他合理邏輯
    ...
    if father.get_color() != 'y' and params['labeled_start_lev'] < l <= params['labeled_stop_lev']+1:
        ...
        # --- 修改開始：只在有標籤內容時才為圖例創建一個隱藏的條形 ---
        if lab: # 只有當 'lab' 不為空字串時，才建立這個圖例條目
            ax.bar(0.0, 0.0, width = 0.0, bottom = 0.0, alpha = 1.0, color=father.get_color(), edgecolor=params['fore_color'], label=lab)
        # --- 修改結束 ---
        if l <= params['abrv_stop_lev'] + 1:
            if not params['col_lab']: col = params['fore_color']
            else:
                # --- 修改開始：確保標籤顏色適用於自定義顏色 ---
                if isinstance(father.get_color(), list): # 如果是 RGB 元組/列表，則標籤預設使用前景色
                    col = params['fore_color']
                elif father.get_color() in colors: # 如果在預設全域顏色列表裡，使用其深色版本
                    col = dark_colors[colors.index(father.get_color()) % len(dark_colors)]
                else: # 否則，使用前景色 (對不在預設列表中的命名顏色安全回退)
                    col = params['fore_color']
                # --- 修改結束 ---
...
def draw_tree(out_file,tree,params):
    ...
    # --- 修改開始：圖例條顏色從 tree['class_to_color_map'] 獲取 ---
    nll = [ax.bar(0.0, 0.0, width = 0.0, bottom = 0.0, color=tree['class_to_color_map'][c], label=c) for c in cll if c in tree['class_to_color_map']]
    # --- 修改結束 ---
